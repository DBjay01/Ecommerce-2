...
const Slider = () => {
  ...
  return (
    <>
      <div className="relative mt-32  md:mx-20 rounded-lg h-[25vh] md:h-[45vh] lg:h-[55vh] overflow-hidden">
        ...
        >
          {slides.map((slide) => (
            ...
            >
              {/* LEFT TEXT CONTENT */}
              <div className="w-1/2 flex flex-col ml-6 justify-center px-6 pr-8 gap-4">
                <h2 className="text-lg md:text-3xl lg:text-4xl font-bold text-orange-600 tracking-wide leading-tight">
                  {slide.title}
                </h2>
                <p className="text-xs md:text-md lg:text-base text-gray-700 font-normal leading-snug line-clamp-2 md:line-clamp-none mr-10">
                  {slide.description}
                </p>
                <Link href={slide.url}>
                  <button className="mt-4 w-fit bg-orange-600 hover:bg-orange-800 text-white text-sm md:text-base px-3 py-2 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300">
                    Explore More
                  </button>
                </Link>
              </div>
              ...
              {/* RIGHT IMAGE */}
              <div className="w-1/2 h-full relative p-4 rounded-xl -ml-10 md:-ml-20 lg:-ml-32">
                <Image
                  src={slide.img}
                  alt={slide.title}
                  fill
        ...
        <div className="absolute left-1/2 bottom-4 -translate-x-1/2 flex gap-3 z-10">
          {slides.map((_, index) => (
            ...
            >
              {current === index && (
                <div className="w-[8px] h-[8px] bg-orange-600 rounded-full"></div>
              )}
            </div>
          ))}
        </div>
      ...
      {/* ADDITIONAL INFO */}
      <div className="hidden sm:flex bg-blue-50 rounded-xl px-6 py-4 mt-6 mx-10 flex-wrap justify-center gap-6 md:gap-12 text-sm md:text-base font-medium text-gray-800">
        <div className="flex items-center gap-2">
          <Box className="w-6 h-6 text-orange-500 flex-shrink-0" />
        ...
        <div className="flex items-center gap-2">
          <BadgeCheck className="w-6 h-6 text-orange-500 flex-shrink-0" />
